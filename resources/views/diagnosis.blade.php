...
<section class="text-center mb-10 lg:mb-16 pt-4 animate-fade-up">
    <div class="inline-flex items-center gap-2 bg-white/70 border border-forest-300 backdrop-blur-md text-forest-800 text-xs font-bold px-4 py-2 rounded-full mb-6 shadow-sm hover:shadow transition-shadow">
        <span class="w-2 h-2 bg-forest-500 rounded-full animate-pulse-slow inline-block"></span>
        Powered by Gemini 2.5 Flash API
    </div>
    <h1 class="text-4xl sm:text-5xl lg:text-7xl font-extrabold text-forest-900 tracking-tighter leading-[1.1] mb-5 drop-shadow-sm">
        <span data-i18n="hero_title_line1">Instant Padi Health</span><br class="hidden sm:block" />
        <span class="text-transparent bg-clip-text bg-gradient-to-br from-forest-600 via-forest-500 to-forest-700 drop-shadow-sm" data-i18n="hero_title_line2">Diagnosis</span>
    </h1>
    <p class="text-forest-800 font-medium text-base sm:text-lg max-w-xl mx-auto leading-relaxed drop-shadow-sm" data-i18n="hero_subtitle">
...
<div class="max-w-3xl mx-auto flex flex-col gap-6 lg:gap-8 transition-all duration-700 ease-in-out" id="dashboard">
    ...
    <div id="leftPanel" class="w-full transition-all duration-700 ease-in-out space-y-6 animate-fade-up delay-200">
        ...
        <div class="glass-card rounded-3xl p-6 shadow-lg">
            <h2 class="font-bold text-forest-900 text-lg mb-1">📸 <span data-i18n="upload_title">Upload atau Ambil Foto</span></h2>
            <p class="text-sm text-forest-600 mb-4" data-i18n="upload_subtitle">Pilih "Live Scan" untuk imbas, atau muat naik foto.</p>

            {{-- Action Buttons --}}
            <div class="flex gap-3 mb-5">
                <button type="button" onclick="startLiveScan()" class="flex-1 bg-forest-600 hover:bg-forest-500 border border-forest-500 hover:shadow-lg text-white py-3 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all hover:-translate-y-0.5">
                    <svg class="w-5 h-5 text-forest-100" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6.827 6.175A2.31 2.31 0 0 1 5.186 7.23c-.38.054-.757.112-1.134.175C2.999 7.58 2.25 8.507 2.25 9.574V18a2.25 2.25 0 0 0 2.25 2.25h15A2.25 2.25 0 0 0 21.75 18V9.574c0-1.067-.75-1.994-1.802-2.169a47.865 47.865 0 0 0-1.134-.175 2.31 2.31 0 0 1-1.64-1.055l-.822-1.316a2.192 2.192 0 0 0-1.736-1.039 48.774 48.774 0 0 0-5.232 0 2.192 2.192 0 0 0-1.736 1.039l-.821 1.316Z" /><path stroke-linecap="round" stroke-linejoin="round" d="M16.5 12.75a4.5 4.5 0 1 1-9 0 4.5 4.5 0 0 1 9 0ZM18.75 10.5h.008v.008h-.008V10.5Z" /></svg>
                    <span data-i18n="btn_live_scan">Live Scan</span>
                </button>
                <button type="button" onclick="document.getElementById('imageInput').click()" class="flex-1 bg-white border border-forest-200 hover:bg-forest-50 hover:shadow-md text-forest-700 py-3 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all hover:-translate-y-0.5">
                    <svg class="w-5 h-5 text-forest-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5m-13.5-9L12 3m0 0 4.5 4.5M12 3v13.5" /></svg>
            ...
            <div id="uploadZone" class="upload-zone rounded-2xl border-2 border-dashed border-forest-400 bg-white/40 p-6 text-center relative overflow-hidden min-h-[220px] flex flex-col justify-center items-center">
                ...
                <div id="uploadPlaceholder" class="relative z-10 pointer-events-none">
                    <div class="w-16 h-16 bg-white border border-forest-200 rounded-full flex items-center justify-center mx-auto mb-3 shadow-sm">
                ...
                <div id="liveControls" class="hidden absolute bottom-4 left-1/2 -translate-x-1/2 z-20 w-fit flex gap-2 justify-center px-4">
                    <button type="button" onclick="captureFrame()" class="bg-white text-forest-700 font-bold px-6 py-2.5 rounded-full shadow-lg border-2 border-forest-500 hover:scale-105 transition-transform flex items-center gap-2">
                        <span class="w-3 h-3 bg-red-500 rounded-full animate-pulse"></span> <span data-i18n="btn_snap">Snap</span>
                    </button>
                    <button type="button" onclick="stopLiveScan()" class="bg-black/50 text-white font-semibold px-4 py-2.5 rounded-full backdrop-blur shadow-lg hover:bg-black/70 transition-colors" data-i18n="btn_cancel">
                        Batal
                    </button>
                </div>

                {{-- Selected file info overlay --}}
                <div id="fileInfo" class="hidden absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-4 z-10 backdrop-blur-sm">
        ...
        {{-- ─── Voice Input Card ──────────────────────────────────────── --}}
        <div class="glass-card rounded-3xl p-6 shadow-lg">
            <h2 class="font-bold text-forest-900 text-base mb-1">🎙️ <span data-i18n="voice_title">Penerangan Suara (Pilihan)</span></h2>
            <p class="text-xs text-forest-600 mb-4" data-i18n="voice_subtitle">Terangkan keadaan tanaman anda dalam Bahasa Melayu. AI akan mengambil kira penerangan ini.</p>

            <div class="flex gap-3 items-start">
                <button id="micBtn" type="button" onclick="toggleSpeech()"
                    ...
                    <svg id="micIcon" class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 18.75a6 6 0 0 0 6-6v-1.5m-6 7.5a6 6 0 0 1-6-6v-1.5m6 7.5v3.75m-3.75 0h7.5M12 15.75a3 3 0 0 1-3-3V4.5a3 3 0 1 1 6 0v8.25a3 3 0 0 1-3 3Z"/>
                    </svg>
                </button>

                <textarea id="voiceText" rows="3"
                          data-i18n-placeholder="voice_placeholder"
        ...
        <button id="newScanBtn" onclick="resetUI()" class="hidden w-full border-2 border-dashed border-forest-400 text-forest-700 bg-white/60 hover:bg-white hover:border-forest-500 backdrop-blur-md shadow-lg font-bold text-sm py-4 rounded-2xl transition-all duration-200" data-i18n="btn_new_scan">
            + Analisis Foto Baru
        </button>
    </div>
    ...
    <div id="rightPanel" class="hidden flex-1 min-w-0 w-full transition-all duration-700 opacity-0 translate-y-4">
        ...
        <div id="loadingState" class="hidden glass-card rounded-3xl p-8 shadow-lg space-y-5">
            <div class="flex items-center gap-3 mb-2">
                <div class="w-8 h-8 bg-forest-200 rounded-full flex items-center justify-center">
                    <svg class="w-4 h-4 text-forest-600 animate-spin-slow" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182m0-4.991v4.99"/>
                    </svg>
                </div>
                <div>
                    <p class="font-semibold text-forest-900 text-sm" data-i18n="loading_title">Menganalisis gambar anda...</p>
        ...
        <div id="errorState" class="hidden glass-card rounded-3xl p-8 shadow-lg border border-red-100">
            ...
            <button onclick="resetUI()" class="mt-2 text-xs text-red-600 underline hover:no-underline" data-i18n="btn_retry">Cuba Semula</button>
        </div>
        ...
        <div id="resultState" class="hidden space-y-5 result-appear">
            ...
            <div class="glass-card rounded-3xl p-6 shadow-lg border border-forest-400/20">
                <h3 class="font-bold text-forest-900 text-base mb-3 flex items-center gap-2">
                    <span class="w-6 h-6 bg-white border border-forest-100 rounded-lg flex items-center justify-center text-sm shadow-sm">🔬</span>
            ...
            <div class="glass-card rounded-3xl p-6 shadow-lg border border-forest-400/20">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div class="step-card bg-blue-50/80 border border-blue-100 rounded-2xl p-4">
                        ...
                        <p class="text-xs font-bold text-blue-800 uppercase tracking-wide mb-2" data-i18n="label_water">Pengairan</p>
                    ...
                    <div class="step-card bg-amber-50/80 border border-amber-100 rounded-2xl p-4">
                        ...
                        <p class="text-xs font-bold text-amber-800 uppercase tracking-wide mb-2" data-i18n="label_fertilizer">Baja</p>
                    ...
                    <div class="step-card bg-red-50/80 border border-red-100 rounded-2xl p-4">
                        ...
                        <p class="text-xs font-bold text-red-800 uppercase tracking-wide mb-2" data-i18n="label_treatment">Rawatan</p>
            ...
            <div id="resourceOptimizationCard" class="hidden glass-card rounded-3xl p-6 shadow-lg border border-teal-400/20 bg-teal-50/30">
                <h3 class="font-bold text-teal-900 text-base mb-3 flex items-center gap-2">
                    <span class="w-6 h-6 bg-white border border-teal-100 rounded-lg flex items-center justify-center text-sm shadow-sm">♻️</span>
            ...
            {{-- Additional Notes --}}
            <div id="additionalNotesCard" class="hidden glass-card rounded-3xl p-6 shadow-lg border border-forest-400/20">
                <h3 class="font-bold text-forest-900 text-base mb-3 flex items-center gap-2">
                    <span class="w-6 h-6 bg-white border border-forest-100 rounded-lg flex items-center justify-center text-sm shadow-sm">📝</span>
