        ...
        <div class="flex items-center gap-3 opacity-0" id="overlayLogo"
            ...
            <div class="w-11 h-11 bg-forest-600 rounded-xl flex items-center justify-center shadow-md">
                <svg class="w-6 h-6 text-white" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M17 8C8 10 5.9 16.17 3.82 21.34L5.71 22l1-2.3A4.49 4.49 0 0 0 8 20c9 0 12-8 12-8-2 2-4 3-6 3a6 6 0 0 1-6-6c0-3.31 2.69-6 6-6s6 2.69 6 6c0 1.39-.47 2.67-1.24 3.69A10 10 0 0 0 17 8z"/>
                </svg>
            </div>
            <div>
                <p class="font-extrabold text-forest-900 text-lg leading-none tracking-tight">ORI<span class="text-forest-500">Padi</span></p>
                <p class="text-forest-500 text-xs font-medium tracking-widest uppercase mt-0.5" data-i18n="overlay_loading">Loading Scanner…</p>
            </div>
        </div>
        {{-- Progress bar --}}
...
<section class="relative bg-forest-50 overflow-hidden min-h-[calc(100vh-4rem)] flex flex-col justify-center">
    ...
    <div class="absolute top-0 left-0 w-[700px] h-[700px] bg-forest-200/50 rounded-full blur-[140px] -translate-x-1/3 -translate-y-1/4 pointer-events-none"></div>
    <div class="absolute bottom-0 right-0 w-[700px] h-[700px] bg-forest-300/35 rounded-full blur-[130px] translate-x-1/3 translate-y-1/4 pointer-events-none"></div>
    <div class="absolute top-1/2 left-1/2 w-[500px] h-[500px] bg-forest-100/60 rounded-full blur-[100px] -translate-x-1/2 -translate-y-1/2 pointer-events-none"></div>

    <div class="relative max-w-7xl mx-auto w-full px-5 sm:px-10 lg:px-16 py-12 lg:py-16 flex flex-col lg:flex-row items-center gap-12 lg:gap-8">

        {{-- ── LEFT TEXT ─────────────────────────────── --}}
        <div class="w-full lg:w-[48%] flex flex-col z-10" style="animation: fadeUp 0.9s ease-out 0.1s both;">

            {{-- Badge --}}
            <div class="inline-flex items-center gap-2 bg-forest-100 border border-forest-200 text-forest-700 text-xs font-bold px-4 py-1.5 rounded-full mb-6 w-fit shadow-sm">
            ...
            <h1 class="text-5xl sm:text-6xl lg:text-7xl font-black text-forest-900 leading-[1.0] tracking-tighter mb-6">
                ...
                <span class="text-transparent bg-clip-text bg-gradient-to-r from-forest-500 to-forest-700" data-i18n="hero_line2">
                    Paddy Crop
                </span><br>
                <span class="text-4xl sm:text-5xl lg:text-6xl text-forest-400 font-black" data-i18n="hero_line3">with AI</span>
            </h1>

            {{-- Subtext --}}
            <p class="text-forest-600 text-base sm:text-lg leading-relaxed mb-8 max-w-md font-medium" data-i18n="hero_sub">
                Snap a photo of your padi plant and get an instant AI diagnosis with a personalised treatment plan — no account needed.
            </p>

            {{-- CTA Row --}}
            <div class="flex flex-wrap items-center gap-4 mb-10">
                <a href="{{ route('scan') }}"
                   onclick="triggerTransition(event, '{{ route('scan') }}')"
                   class="group inline-flex items-center gap-3 bg-forest-600 hover:bg-forest-700 text-white font-extrabold text-base px-8 py-4 rounded-full shadow-xl shadow-forest-600/30 transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-forest-600/40">
                    ...
                    <span class="w-7 h-7 bg-white/20 rounded-full flex items-center justify-center group-hover:bg-white/30 transition-colors flex-shrink-0">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3"/>
                        </svg>
                    </span>
                </a>
                <a href="#how" class="text-forest-600 hover:text-forest-800 font-semibold text-sm flex items-center gap-1.5 transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path stroke-linecap="round" d="M12 8v4l2 2"/></svg>
            ...
            <div class="flex items-center gap-6 flex-wrap">
                <div>
                    ...
                    <p class="text-forest-500 text-xs font-medium tracking-wide mt-0.5" data-i18n="stat1">Diseases Detected</p>
                </div>
        ...
        <div class="w-full lg:w-[52%] relative z-10 flex justify-center lg:justify-end items-stretch self-stretch"
            ...
            <div class="relative w-full max-w-lg lg:max-w-none flex flex-col">
                {{-- Accent block behind --}}
                <div class="absolute -bottom-4 -right-4 w-full h-full bg-forest-200 rounded-[2rem] z-0 hidden lg:block"></div>

                {{-- Main image --}}
                <div class="relative z-10 rounded-[2rem] overflow-hidden shadow-2xl shadow-forest-900/20 border-4 border-white flex-1">
                    <img src="/images/paddy_hero.png"
                         alt="Lush green paddy rice field"
                    ...
                    <div class="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-forest-900/40 to-transparent"></div>
...
<section id="how" class="bg-white py-20 px-5 sm:px-10 lg:px-16">
    <div class="max-w-5xl mx-auto">
        <div class="text-center mb-14">
            <p class="text-forest-500 text-xs font-bold uppercase tracking-widest mb-2" data-i18n="how_eyebrow">Simple & Fast</p>
            <h2 class="text-3xl sm:text-4xl font-black text-forest-900 tracking-tight" data-i18n="how_title">How It Works</h2>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-8">
            <div class="relative bg-forest-50 border border-forest-100 rounded-3xl p-7 hover:shadow-lg hover:border-forest-200 transition-all duration-200 group">
                <div class="w-12 h-12 bg-forest-100 group-hover:bg-forest-200 rounded-2xl flex items-center justify-center text-2xl mb-5 transition-colors">📸</div>
                <p class="text-forest-400 text-xs font-bold uppercase tracking-widest mb-1" data-i18n="step1_num">Step 1</p>
                <h3 class="text-forest-900 font-extrabold text-lg mb-2" data-i18n="step1_title">Snap a Photo</h3>
                <p class="text-forest-600 text-sm leading-relaxed" data-i18n="step1_desc">Upload or live-scan your padi crop. Works with any phone camera.</p>
            </div>
            <div class="relative bg-forest-50 border border-forest-100 rounded-3xl p-7 hover:shadow-lg hover:border-forest-200 transition-all duration-200 group">
                <div class="w-12 h-12 bg-forest-100 group-hover:bg-forest-200 rounded-2xl flex items-center justify-center text-2xl mb-5 transition-colors">🤖</div>
                <p class="text-forest-400 text-xs font-bold uppercase tracking-widest mb-1" data-i18n="step2_num">Step 2</p>
                <h3 class="text-forest-900 font-extrabold text-lg mb-2" data-i18n="step2_title">AI Diagnoses</h3>
                <p class="text-forest-600 text-sm leading-relaxed" data-i18n="step2_desc">Gemini AI identifies the disease and severity in under 3 seconds.</p>
            </div>
            <div class="relative bg-forest-50 border border-forest-100 rounded-3xl p-7 hover:shadow-lg hover:border-forest-200 transition-all duration-200 group">
                <div class="w-12 h-12 bg-forest-100 group-hover:bg-forest-200 rounded-2xl flex items-center justify-center text-2xl mb-5 transition-colors">📋</div>
                <p class="text-forest-400 text-xs font-bold uppercase tracking-widest mb-1" data-i18n="step3_num">Step 3</p>
                <h3 class="text-forest-900 font-extrabold text-lg mb-2" data-i18n="step3_title">Get Your Plan</h3>
                <p class="text-forest-600 text-sm leading-relaxed" data-i18n="step3_desc">Receive a full treatment plan for water, fertiliser & pest control.</p>
            </div>
        </div>
...
<footer class="bg-forest-50 border-t border-forest-200/60 py-8 px-5 text-center">
    <p class="text-forest-500 text-sm">
        <span class="text-forest-800 font-bold">ORIPadi</span> &mdash; <span data-i18n="footer_text">A public tool for Malaysian rice farmers</span> &bull;
...
<style>
    ...
    [style*="animation: fadeUp"] { opacity: 0; }
</style>
